...
INFECTION_PROB = 0.2
RECOVERY_TIME = 300
DEATH_PROB = 0.05  # 5% chance agent dies instead of recovering
...
def animate(frame):
    ...
    legend_labels["S"].set_text(f"Susceptible: {counts[SUSCEPTIBLE]}")
    legend_labels["I"].set_text(f"Infected:    {counts[INFECTED]}")
    legend_labels["R"].set_text(f"Recovered:   {counts[RECOVERED]}")
    legend_labels["D"].set_text(f"Dead:        {counts[DEAD]}")

    # Return all elements that change (for blitting)
...
# Export epidemic data after animation finishes
